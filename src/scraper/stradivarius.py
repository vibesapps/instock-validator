...
_API_KEYWORDS = ("product", "catalog", "detail", "price", "stock", "itxrest")
...
class StradivariusScraper:
    ...
    async def scrape(self, product: Product) -> ScrapeResult:
        ...
        async def on_response(resp: Response) -> None:
            ...
            try:
                ...
                if isinstance(data, dict):
                    ...
                    # Stradivarius sometimes returns a top-level array
                    for item in data:
        ...
        try:
            async with self._browser.new_page() as page:
                ...
                try:
                    await page.wait_for_selector(
                        "h1, [data-qa-id], .product-detail-view",
                        timeout=25_000,
                    )
                except Exception:
                    pass
                ...
                if ban_signal:
                    log.warning("[strad] Ban: product=%s signal=%s", product.name, ban_signal)
                    return ScrapeResult(
                ...
                log.info(
                    "[strad] OK: %s / %s | in_stock=%s price=%s",
            ...
            log.warning("[strad] Error scraping %s: %s", product.url, exc)
            return ScrapeResult(
                product_id=product.id,
                timestamp=datetime.now(timezone.utc),
                success=False,
